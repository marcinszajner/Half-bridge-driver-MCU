...
void execute_protocol(void* ptr)
{
  ...
  case 0xaa:
      ...
      {
      case UPDATE:
        if((((protocol_change*)protocol_ptr)->bitfield & FREQUENCY_BIT) == FREQUENCY_BIT)
        {
      ...
      case DATA_REQ:
      {
        if(crc_check(ptr))
        {
          //TODO przekazac bezposrednio pointer bez struktury dodatkowej (przekazac requesta)
          init_data_acq_t init_data_acq;
          init_data_acq.fequency_step = ((protocol_data_req*)protocol_ptr)->fequency_step;
          init_data_acq.packet_size = ((protocol_data_req*)protocol_ptr)->packet_size;
          init_data_acq.n_packet = ((protocol_data_req*)protocol_ptr)->n_packet;
          init_data_acq.actual_frequency = ((protocol_data_req*)protocol_ptr)->actual_frequency;

          change_frequency(((protocol_data_req*)protocol_ptr)->actual_frequency);

          init_data_acq_struct(&init_data_acq);
        }
        else
        {
...
{
  protocol_data_resp* adc_data_resp = (protocol_data_resp*)get_adc_data_orgin();
  adc_data_resp->head.signature = 0xAA;
  adc_data_resp->head.size = sizeof(protocol_data_resp) + data_acq_con_ar->packet_size*SAMPLE_SIZE*data_acq_con_ar->n_packet + END_SIGNATURE_SIZE + 2; // TODO check why + 2 needed
  ...
  adc_data_resp->actual_frequency = data_acq_con_ar->actual_frequency + (n_packet)*frequency_step;

  adc_data_resp->packet_size = packet_size;
  adc_data_resp->n_packet = n_packet;
  adc_data_resp->fequency_step = frequency_step;

  uint32_t* end_singature_ptr = (uint32_t*)&adc_data_resp->payload[(packet_size*SAMPLE_SIZE*n_packet)+2];
